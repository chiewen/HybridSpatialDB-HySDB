...
SecondaryElement::SecondaryElement(Bucket* p_bucket, unsigned index, int row, int col)
	: p_bucket(p_bucket), index(index), row(row), col(col),
	p_bucket_ld(nullptr), index_ld(0), row_ld(-1), col_ld(-1) {}

void SecondaryElement::RollInValues(Bucket* p_bucket, unsigned index, int row, int col) {
